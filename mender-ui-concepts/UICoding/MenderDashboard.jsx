...
// Mender brand color
const MENDER_GREEN = '#00FF66';

const MenderDashboard = () => {
  ...
  const getConditionStyle = (condition) => {
    const styles = {
      A: { background: 'rgba(0, 255, 102, 0.15)', color: MENDER_GREEN, border: `1px solid rgba(0, 255, 102, 0.4)` },
      B: { background: 'rgba(251, 191, 36, 0.15)', color: '#FBBF24', border: '1px solid rgba(251, 191, 36, 0.4)' },
      C: { background: 'rgba(251, 113, 133, 0.15)', color: '#FB7185', border: '1px solid rgba(251, 113, 133, 0.4)' },
    };
    return styles[condition] || { background: 'rgba(100, 116, 139, 0.2)', color: '#94A3B8' };
  };
  
  const getActionStyle = (action) => {
    const styles = {
      Redeploy: MENDER_GREEN,
      Refurb: '#38BDF8',
      Recycle: '#FBBF24',
    };
    return styles[action] || '#94A3B8';
  };
  ...
  return (
    <div className="min-h-screen bg-black flex font-sans">
      {/* Sidebar */}
      <aside className="w-60 bg-neutral-950 border-r border-neutral-800 flex flex-col">
        {/* Customer Logo */}
        ...
        <nav className="flex-1 p-3 pt-4">
          {navItems.map((item) => (
            <button
              ...
              style={
                activeNav === item
                  ? { background: 'rgba(0, 255, 102, 0.1)', color: MENDER_GREEN, borderLeft: `2px solid ${MENDER_GREEN}` }
                  : { color: '#A3A3A3' }
              }
              onMouseEnter={(e) => {
                if (activeNav !== item) {
        ...
        <div className="p-4 border-t border-neutral-800">
          <div className="flex items-center gap-3">
            ...
            >
              <span className="text-xs text-black font-medium">JD</span>
            </div>
      ...
      <main className="flex-1 flex flex-col">
        {/* Header */}
        <header className="h-16 border-b border-neutral-800 flex items-center justify-between px-8 bg-neutral-950">
          <h1 className="text-xl text-white font-semibold tracking-tight">
            Dashboard
          </h1>
          <div className="flex items-center gap-4">
            <span className="text-xs text-neutral-500 font-mono">Last sync: 2 min ago</span>
            <button 
              className="px-4 py-1.5 text-xs font-semibold rounded transition-colors"
              style={{ background: MENDER_GREEN, color: '#000000' }}
              onMouseEnter={(e) => e.currentTarget.style.background = '#33FF85'}
        ...
        <div className="flex-1 p-8 overflow-auto flex flex-col">
          ...
          <div className="grid grid-cols-3 gap-6 mb-8">
            {summaryCards.map((card, index) => (
              <div
                key={card.label}
                className="bg-neutral-950 border border-neutral-800 rounded-xl p-6 hover:border-neutral-700 transition-all duration-300 group"
              ...
              >
                ...
                <div className="flex items-baseline gap-1">
                  ...
                  {card.subtext && (
                    <span className="text-sm text-neutral-500 font-medium">{card.subtext}</span>
                  )}
                </div>
              </div>
            ))}
          ...
          <div className="bg-neutral-950 border border-neutral-800 rounded-xl overflow-hidden flex-1">
            <div className="px-6 py-4 border-b border-neutral-800 flex items-center justify-between">
              ...
              <button 
                ...
                onMouseEnter={(e) => e.currentTarget.style.color = '#33FF85'}
                  ...
                  {recentValuations.map((item, index) => (
                    ...
                    >
                      <td className="px-6 py-4">
                        <span 
                          className="text-sm text-white transition-colors group-hover:text-current"
                          style={{ '--hover-color': MENDER_GREEN }}
          ...
          <div className="flex justify-end mt-6">
            <div className="flex items-center gap-1.5 text-neutral-500">
              ...
              <span className="text-white font-semibold text-sm">mender</span>
...
export default MenderDashboard;
